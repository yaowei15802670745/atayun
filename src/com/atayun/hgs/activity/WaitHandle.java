					...
					public void onResponse(JSONObject response) {
						try {
							if (response == null) {
								...
								Toast.makeText(WaitHandle.this, "获取数据成功",
										Toast.LENGTH_SHORT).show();
								...
								case "200":
									Toast.makeText(WaitHandle.this, "登录成功",
											Toast.LENGTH_SHORT).show();
									JSONArray OrderList = response
...
//									String orderNo=OrderList.getJSONObject(0).getString("orderNo");
									
									

									break;
								case "10":
									Toast.makeText(WaitHandle.this, "没有未完成订单",
											Toast.LENGTH_SHORT).show();
									break;
					...
					public void onErrorResponse(VolleyError error) {
						Toast.makeText(WaitHandle.this, "获取失败，请检查网络",
								Toast.LENGTH_SHORT).show();
...
//		listview.setOnItemClickListener(new ItemClickEvent());
	...
	private List<Map<String, Object>> getData() {
		...
		if(ordermanagerIterm.size()>0){
			...
			for (int i = 0; i < ordermanagerIterm.size(); i++) {
				...
				map.put("id", i);
				// 把listview显示的内容存入
				map.put("订单号", ordermanagerIterm.get(i).getOrderNo());
				map.put("下单时间", ordermanagerIterm.get(i).getUpdateTime());
				map.put("价格", ordermanagerIterm.get(i).getOrderPrice());
	...
	public class ItemClickEvent implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
			...
			Intent intent = new Intent(WaitHandle.this, OrderWaitActivity.class);
